...
ggplot(data, aes(k, alpha, fill=Probability)) + geom_tile()+scale_fill_viridis(limits=limits,breaks=limits[1]:limits[2],labels=c('<0.0001','0.001','0.01','0.1','1'))+
  ...
  geom_segment(aes(x = -3, xend = 10.5, y = 3.5, yend = 3.5), linetype="dashed", color = "red")+
  geom_segment(aes(x = -3, xend = 10.5, y = 6.5, yend = 6.5), linetype="dashed", color = "red")+
  geom_segment(aes(x = -3, xend = 10.5, y = 9.5, yend = 9.5), linetype="dashed", color = "red")+
