...
{\color{red} Don't use all of these: -Ian}
	\begin{align}
		p_{k,t}({\bf s}_t, N_t)
			...
			& = \sum_{i=1}^{N_t} \frac{s_{t,i}! (N_{t+1}-k)!}{(s_{t,i}-k)! N_{t+1}!} \\
...
{\color{red}
	...
	\begin{itemize}
		...
		\item Sum subscripts over ${\bf s}_t$ rather than ${\bf s}_t : ...$ (at least L1-3)? We have probability 0 if sum does not equal $N_{t+1}$ anyway
...
and
	...
	\]
Analogously to the Wright-Fisher model in which individuals select a parent uniformly at random from the previous generation, we find that the inclusive probability of coalescence for two lines is
%
