...
for future studies of infectious disease outbreaks.
...
We define the inclusive coalescence probability $p_{k}(N_t, N_{t+1})$ as the probability that a specific set of $k$ individuals from generation $t+1$ find a common ancestor in generation $t$, conditional on population sizes $N_t$ and $N_{t+1}$.
...
but this is not feasible to observe in practice.
...
%		& = \sum_{{\bf s}_t : \sum_{i=1}^{N_t} s_{i,t} = N_{t+1}} 
		& = \sum_{{\bf s}_t \in \mathbb{N}_0^{N_t}} \mathbb{P} \bigg[{\bf S}_t = {\bf s}_t \bigg| \sum_{i=1}^{N_t} S_{t,i} = N_{t+1} \bigg] p_{k}({\bf s}_t, N_t) \nonumber\\
%		& = \sum_{{\bf s}_t : \sum_{i=1}^{N_t} s_{i,t} = N_{t+1}} 
...
In this case, we are interested in the exclusive coalescence probability $p_{n,k}(N_t, N_{t+1})$ that exactly $k$ individuals from a sample of $n$ arose from a common ancestor one generation in the past given knowlege of the total population sizes $N_t$ and $N_{t+1}$.
...
Similarly to the exclusive coalescence probability, we can use this to evaluate the exclusive probability given $N_t$ and $N_{t+1}$ by summing over possible parent offspring configurations (for $k \leq n$):
	...
	\begin{align}
		p_{n,k}(N_t, N_{t+1})
			& = \sum_{{\bf x}_t \in \mathbb{N}_0^{N_t}} \mathbb{P} \bigg[ {\bf X}_t = {\bf x}_t \bigg | \sum_{i=1}^{n} X_{t,i} = n \bigg] p_{n,k} ({\bf x}_t, N_t) \nonumber\\
...
(with probability $p_{n,1}(N_t, N_{t+1})$) or coalesce in an event of size $k$ (with probability $p_{n,k}(N_t, N_{t+1})$) with any set of $k-1$ lines among the $n-1$ other lines, leading to the following complementarity equation: 

\begin{equation}
\sum_{k=1}^n \binom{n-1}{k-1} p_{n,k}(N_t, N_{t+1}) =1
\end{equation}

We can show that it is indeed satisfied by the formula in Equation \eqref{eq:GeneralExclusiveProb}:

	\begin{align}
		\sum_{k=1}^n \binom{n-1}{k-1} p_{n,k}(N_t, N_{t+1})
...
\begin{equation}
	p_{k}(N_t, N_{t+1})=\frac{1}{N_t^{k-1}}\label{eq:PoisInclusiveProb}
\end{equation}

In addition, by injecting the probability mass function of the Binomial distribution in
...
the exclusive probability of coalescence for $k$ lines from a sample of $n$ $(n \geq k)$ is:
	\begin{equation}
		p_{n,k}(N_t, N_{t+1}) = \frac{(N_t-1)^{n-k}}{N_t^{n-1}}\label{eq:PoisExclusiveProb}
	\end{equation}
...
In addition, by injecting the probability mass function of the Beta-Binomial distribution in Equation
\eqref{eq:beta-binomial} into Equation 
\eqref{eq:GeneralExclusiveProb} we deduce that 
the exclusive probability of coalescence for $k$ lines is:

\begin{equation}
p_{n,k}(N_t, N_{t+1})=\frac{N_t \mathrm{B}(k+r, n-k+N_t r-r)}{\mathrm{B}(r, N_t r-r)}
...
\begin{equation}
p_{2}(N_t, N_{t+1})=p_{n,2}(N_t, N_{t+1})=\frac{1}{N_t}\label{eq:poissonapprox}
\end{equation}
...
\begin{equation}
p_{2}(N_t, N_{t+1})=p_{n,2}(N_t, N_{t+1})=\frac{r+1}{N_t r +1} \approx \frac{r+1}{N_t r}\label{eq:negbinapprox}
\end{equation}
...
\begin{equation}
 p_{2}(N_t, N_{t+1})=\frac{\sigma^2/R+R-1}{N_t R}\label{eq:fraser}
 \end{equation}
...
The exclusive coalescent probability $p_{n,k}(N_t, N_{t+1})$ in the Negative-Binomial case 
...
\begin{equation}
\lambda_{n,k}=-\mathrm{log}(1-p_{n,k}(N_t, N_{t+1}))
\label{eq:omega}
\end{equation}
