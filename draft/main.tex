...
We can show that it is indeed satisfied by the formula in Equation \ref{eq:GeneralExclusiveProb}:
...
\ref{eq:GeneralExclusiveProb} we deduce that 
...
The inclusive coalescent probability (Equation \ref{eq:PoisInclusiveProb}) 
...
is $1/N_t$. The exclusive coalescent probability (Equation \ref{eq:PoisExclusiveProb}) 
...
By injecting this formula in Equation \ref{eq:GeneralInclusiveProb}, we deduce that the inclusive probability of coalescence for $k$ lines is:
...
Here we can relate the inclusive coalescent probability for two lines $p_{2,t}$ to previous results from
