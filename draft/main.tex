...
\noindent where  $\mathrm{B}(x,y)$ denotes the Beta function defined as $\mathrm{B}(x,y)=\Gamma(x)\Gamma(y)/\Gamma(x+y)=\int t^{x-1} (1-t)^{y-1} \mathrm{d}t$. This is the probability mass function of a Beta-Binomial distribution and therefore we deduce that:
...
the probability of multiple merger events of various sizes
