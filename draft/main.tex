...
Note that this equation implies that time is measured
continuously in number of transmission generations. 
...
The omega-coalescent has two parameters: the constant population size $N_t$ and
...
$n=100$ leaves, with parameter $N_t$ drawn uniformly at random between 100 and 500
...
parameters were $N_t=200$ and $r=0.5$, 
...
which is relevant to analysis of outbreaks of a relatively small size,
and to diseases with high transmission heterogeneity.
This process can be incorporated into a new lambda-coalescent
...
described lambda-coalescent model are mostly
concerned with situations where an individual can be the father of a significant
portion of a population in spite of the population being large, rather than 
small populations with superspreading as we modelled here.
...
outbreak, which we hope will be useful especially in settings where 
