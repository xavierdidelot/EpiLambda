...
\begin{document}
{\Large Coalescent process for epidemic models with superspreading}
...
\vspace*{2cm}
Running title: Coalescent for epidemic with superspreading

\vspace*{2cm}
Keywords: infectious disease epidemiology modelling; offspring distribution; superspreading; lambda-coalescent model; multiple mergers
...
The inclusive probability of coalescence for $k$ lines is given by Equation \eqref{eq:GeneralInclusiveProb} by injecting the falling factorial moments of a Binomial distribution 
\citep{Potts1953}, resulting in:
\begin{equation}
...
The factorial moments of $X \sim \mathrm{BetaBinomial}(\alpha,\beta)$ are \citep{Tripathi1994}:
...
By injecting this formula in Equation \eqref{eq:GeneralInclusiveProb}, we deduce that the inclusive probability of coalescence for $k$ lines is:
...
Let the offspring distribution have a mean of $R_t=2$. In the Poisson case the offspring distribution is Poisson(2). We consider NegBin cases with the same mean and varying dispersion parameters $r$. 
When $r=1$ the NegBin reduces to a Geometric distribution.
When $r$ is high the dispersion is low and the NegBin behaves almost like a Poisson.
...
Then we can relate the inclusive coalescent probability for two lines $p_{2,t}$ to 
previous results from
\citep{Volz2012a}, \citep{koelleRatesCoalescenceCommon2012} and
 \citep{Li2017,Fraser2017}.
 
\citep{Fraser2017} have as their main result an estimate of the effective population size $N_e(t)$ 
...
\begin{equation}
 p_{2,t}=\frac{V_t/R_t+R_t-1}{N_t R_t}
 \end{equation}

In the Poisson case we have $V_t=R_t$ so this reduces to $p_{2,t}=1/N_t$.

For the Negative-Binomial case we have $V_t/R_t=1/p=(r+R_t)/r$ so that:

\begin{equation}
p_{2,t}=\frac{V_t/R_t+R_t-1}{N_t R_t}=\frac{r+R_t+rR_t-r}{rN_t R_t}=\frac{r+1}{rN_t}
\end{equation}

which is the same as in Equation \eqref{eq:negbinapprox}.
